...
function convertToEmployees(data: ExcelRow[]): Employee[] {
  ...
  for (const row of data) {
    ...
    const jobNumber = findColumnValue(row, ['رقم_الوظيفة', 'jobNumber', 'رقم الوظيفة', 'الرقم الوظيفي', 'رقم الوظيفي', 'رقم الوظيفة', 'الرقم']);
    const name = findColumnValue(row, ['اسم_الموظف', 'name', 'اسم الموظف', 'الاسم', 'اسم العامل', 'اسم العامل', 'الاسم الكامل']);
    const regionName = findColumnValue(row, ['المنطقة', 'region', 'regionName', 'المنطقة المخصصة', 'المنطقة', 'منطقة العمل']);
    const salary = findColumnValue(row, ['الراتب_الاساسي', 'baseSalary', 'الراتب الأساسي', 'الراتب', 'الراتب الشهري', 'الراتب', 'الراتب الأساسي']);
...
async function main() {
  ...
  if (!filePath) {
    ...
    console.log('مثال: npm run import-real-data-fixed ./5-2025.xlsx');
    process.exit(1);
  }
